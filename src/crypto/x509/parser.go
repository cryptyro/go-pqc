...
func parsePublicKey(keyData *publicKeyInfo) (any, error) {
	...
	case oid.Equal(oidPublicKeyEDMLDSA):
		if len(params.FullBytes) != 0 {
			return nil, errors.New("x509: EDMLDSA key encoded with illegal parameters")
		}
		if len(der) != xdsa.PublicKeySize {
			return nil, errors.New("x509: wrong EDMLDSA public key size")
		}
		// Parse the DER-encoded public key into an mldsa.PublicKey
		var pub xdsa.PublicKey
		err := pub.UnmarshalBinary(der)
		if err != nil {
			return nil, fmt.Errorf("x509: failed to unmarshal EDMLDSA public key: %v", err)
		}
		return &pub, nil
