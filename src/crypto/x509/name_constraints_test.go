...
func makeConstraintsCACert(constraints constraintsSpec, name string, key *xdsa.PrivateKey, parent *Certificate, parentKey *xdsa.PrivateKey) (*Certificate, error) {
	...
	derBytes, err := CreateCertificate(rand.Reader, template, parent, key.Public(), parentKey)
	if err != nil {
		return nil, err
	}
...
func makeConstraintsLeafCert(leaf leafSpec, key *xdsa.PrivateKey, parent *Certificate, parentKey *xdsa.PrivateKey) (*Certificate, error) {
	var serialBytes [16]byte
	rand.Read(serialBytes[:])
...
func TestConstraintCases(t *testing.T) {
	...
	for i, test := range nameConstraintsTests {
		t.Run(fmt.Sprintf("#%d", i), func(t *testing.T) {
			...
			rootKey := privateKeys.Get().(*xdsa.PrivateKey)
			...
			for level, intermediates := range test.intermediates {
				levelKey := privateKeys.Get().(*xdsa.PrivateKey)
			...
			leafKey := privateKeys.Get().(*xdsa.PrivateKey)
