...
func ParsePKCS8PrivateKey(der []byte) (key any, err error) {
	...
	case privKey.Algo.Algorithm.Equal(oidPublicKeyEDMLDSA):
		if l := len(privKey.Algo.Parameters.FullBytes); l != 0 {
			return nil, errors.New("x509: invalid EDMLDSA private key parameters")
		}
		var PrivateKey xdsa.PrivateKey
		if err := (&PrivateKey).UnmarshalBinary(privKey.PrivateKey); err != nil {
			return nil, fmt.Errorf("x509: invalid EDMLDSA private key: %v", err)
		}
		if l := len(privKey.PrivateKey); l != xdsa.PrivateKeySize {
			return nil, fmt.Errorf("x509: invalid EDMLDSA private key length: %d", l)
		}
...
func MarshalPKCS8PrivateKey(key any) ([]byte, error) {
	...
	case *xdsa.PrivateKey:
		privKey.Algo = pkix.AlgorithmIdentifier{
			Algorithm: oidPublicKeyEDMLDSA,
		}

		var err error
		privKey.PrivateKey, err = k.MarshalBinary()
		if err != nil {
			return nil, errors.New("x509: failed to marshal Hybrid private key while building PKCS#8: " + err.Error())
		}
