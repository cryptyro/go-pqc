...
func (c *Conn) verifyServerCertificate(certificates [][]byte) error {
	...
	case *rsa.PublicKey, *ecdsa.PublicKey, ed25519.PublicKey, *xdsa.PublicKey:
		break
	default:
...
func certificateRequestInfoFromMsg(ctx context.Context, vers uint16, certReq *certificateRequestMsg) *CertificateRequestInfo {
	...
	if !certReq.hasSignatureAlgorithm {
		...
		case rsaAvail && ecAvail:
			cri.SignatureSchemes = []SignatureScheme{
				...
				PKCS1WithSHA256, PKCS1WithSHA384, PKCS1WithSHA512, PKCS1WithSHA1, PureEDMLDSA,
			}
		case rsaAvail:
			cri.SignatureSchemes = []SignatureScheme{
		...
		case ecAvail:
			cri.SignatureSchemes = []SignatureScheme{
				ECDSAWithP256AndSHA256, ECDSAWithP384AndSHA384, ECDSAWithP521AndSHA512, PureEDMLDSA,
			}
		}
		return cri
	}
	...
	for _, sigScheme := range certReq.supportedSignatureAlgorithms {
		...
		switch sigType {
		case signatureECDSA, signatureEd25519, signatureEDMLDSA:
			if ecAvail {
